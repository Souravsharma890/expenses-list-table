...
add_btn.addEventListener("click", () => {
  ...
  category_cell.textContent = expenseLists[count].category_val;
  const amount_cell = table_row.insertCell();
  amount_cell.setAttribute("id", `amt_${count}`);
  amount_cell.textContent = expenseLists[count].amount_val;
  const date_cell = table_row.insertCell();
  date_cell.setAttribute("id", `date_${count}`);
  date_cell.textContent = expenseLists[count].date_val;
  const del_cell = table_row.insertCell();
  del_cell.setAttribute("id", `del_${count}`);
  count++;
  console.log(count);
